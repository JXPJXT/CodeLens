...
function showPanel(extensionUri, data) {
    if (currentPanel) {
        currentPanel.reveal(vscode.ViewColumn.Two);
    }
    else {
        currentPanel = vscode.window.createWebviewPanel('codelensReview', 'CodeLens Review', vscode.ViewColumn.Two, { enableScripts: true });
        currentPanel.onDidDispose(() => { currentPanel = undefined; });
        currentPanel.webview.onDidReceiveMessage(message => {
            if (message.command === 'feedback') {
                (0, api_1.sendFeedback)(data.code, data.score, message.correct);
...
function getWebviewContent(data) {
    ...
    if (data.type === 'scan') {
        ...
        const highCount = riskyLines.filter((r) => r.score > 0.7).length;
        const medCount = riskyLines.filter((r) => r.score > 0.3 && r.score <= 0.7).length;
        const lowCount = riskyLines.filter((r) => r.score <= 0.3).length;
        const lineItems = riskyLines.map((r) => {
